...
这里，`TypeHandler`模板类针对通用类型有一个默认实现，而针对`int`类型进行了特化。当调用`TypeHandler<int>::process()`时，会执行特化版本的函数；调用`TypeHandler<double>::process()`时，执行通用版本。在模板特化中，不同的模板参数类型就如同`switch - case`中不同的`case`值，模板的通用实现类似于`switch - case`中的`default`分支。它们都提供了一种根据不同条件选择执行不同代码块的方式。
...
这个时候回头看这个例子，当`T`是整数类型时，第一个`process`函数模板实例化成功，第二个失败，调用第一个；当`T`不是整数类型时，第一个实例化失败，第二个成功，调用第二个。这样就实现了类似于`if-else`的功能。
...
至于`std::is_integral_v<T>`这个怎么做，实际上就是等价于对类型`T`进行值操作，判断它是不是整数类型。前面有提过，模板具备对类型、非类型有一定的操作能力的，这里先不展开讲了。
...
`concept`的方式确实是进一步增强了模板元的`if(condition)`能力的，但笔者的生产环境没到`C++20`，用得不多，写得不多，就不深入介绍。
