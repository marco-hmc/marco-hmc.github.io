...
程序管理资源文件（如图片、音频、配置文件、字符串表等）是一项非常常见的任务，不同平台、语言和场景下的做法会有所不同。Qt 作为跨平台框架，提供了完整的资源管理解决方案，既支持传统的资源管理方式，也有自己独特的 QRC 资源系统。
...
Qt 的资源系统是其最具特色的资源管理方式，通过 QRC 文件实现资源的编译时打包和运行时访问。
